...
public class SearchParameterStepDef {
    ...
    @Then("User gets more than {int} links on the first page")
    public void user_gets_more_than_links_on_the_first_page(int link) {
        Assert.assertTrue(googleMainPage.validateNumberOfLinks(link));
    }

    @When("User navigates for {string}")
    public void user_navigates_for(String string) {
    }
